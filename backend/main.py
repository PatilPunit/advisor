...
def register(request: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    return schemas.UserOut(
        id=user.id, name=user.name, email=user.email, career_goal=user.career_goal,
        role=user.role, created_at=user.created_at, access_token=token,
    )
...
def set_goal(user_id: int, request: schemas.SetGoalRequest, db: Session = Depends(get_db), current_user=Depends(verify_same_user_or_admin)):
    try:
...
def list_skills(user_id: int, db: Session = Depends(get_db), current_user=Depends(verify_same_user_or_admin)):
    return crud.get_user_skills(db, user_id)


@app.post("/users/{user_id}/skills", response_model=schemas.SkillOut)
def update_skill(user_id: int, request: schemas.SkillUpdateRequest, db: Session = Depends(get_db), current_user=Depends(verify_same_user_or_admin)):
...
def list_projects(user_id: int, db: Session = Depends(get_db), current_user=Depends(verify_same_user_or_admin)):
    return crud.get_user_projects(db, user_id)


@app.post("/users/{user_id}/projects", response_model=schemas.ProjectOut)
def update_project(user_id: int, request: schemas.ProjectUpdateRequest, db: Session = Depends(get_db), current_user=Depends(verify_same_user_or_admin)):
...
def learning_report(user_id: int, db: Session = Depends(get_db), current_user=Depends(verify_same_user_or_admin)):
...
def notifications(user_id: int, db: Session = Depends(get_db), current_user=Depends(verify_same_user_or_admin)):
